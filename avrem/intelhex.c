	...
	do {
		...
		if (fgets(line, sizeof(line) / sizeof(line[0]), ihex) != NULL) {
			...
			if (line_len < LINE_MIN_LEN) {
				printl(LOG_ERROR, "Invalid line %u of Intel Hex file; incomplete record, not enough characters.\n", line_count);
				return -1;
			}

#ifdef IHEX_DETAIL
			printl(LOG_DETAIL2, "Line %u: \"%s\" (%zu chars)\n", line_count, line, line_len);
#endif

			// Decode the line into a record (i.e. byte array).
			record_len = intelhex_decode_record(line, line_len, record, sizeof(record));
			if (record_len == 0) {
				printl(LOG_ERROR, "Invalid line %u of Intel Hex file; error parsing hex digits.\n", line_count);
				return -1;
			}

			// Verify that record length and byte count field correlate.
			if ((size_t)record[REC_OFFSET_BYTE_COUNT] + 5 != record_len) {
				printl(LOG_ERROR, "Invalid line %u of Intel Hex file; byte count field value does not correlate with record length.\n", line_count);
				return -1;
			}
...
#ifdef IHEX_DETAIL
			printl(LOG_DETAIL2, "Checksum: Record = 0x%02X, Calculated = 0x%02X\n", record[record_len - 1], checksum);
#endif

			// Verify calculated checksum matches record's.
			if (checksum != record[record_len - 1]) {
				printl(LOG_ERROR, "Invalid line %u of Intel Hex file; failed checksum verification (got %02X, expected %02X).\n", line_count, checksum, record[record_len - 1]);
				return -1;
			}
			...
			case REC_TYPE_DATA:
				...
				//printf("Write at %08X: %u bytes\n", mem_addr, record[REC_OFFSET_BYTE_COUNT]);
				for (i = 0; i < record[REC_OFFSET_BYTE_COUNT]; i++) {
					avr_flash_write_byte(core, mem_addr + i, record[REC_OFFSET_DATA + i]);
					totalload++;
				}
				//printf("\n");
				break;
...
#ifdef IHEX_DETAIL
				printl(LOG_DETAIL2, "Offset: Addr = 0x%08X\n", mem_offset);
#endif
				break;
			...
			case REC_TYPE_START_LIN_ADDR:
				...
				break;
			default:
				printl(LOG_ERROR, "Invalid line %u of Intel Hex file; unknown record type (%u).\n", line_count, record[REC_OFFSET_TYPE]);
				return -1;
			}
		}

		if (ferror(ihex)) {
			printl(LOG_DETAIL2, "Error reading file!\n"); // TODO: print err msg with strerror()?
			return -1;
		}
	} while (!end && !feof(ihex));
